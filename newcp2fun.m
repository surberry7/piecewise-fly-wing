...
% July 2017
% adapted from David Houle lab

% This script is a modification of cp2fun which was originally written by the Houle Lab
% newcp2fun reads one CP file (format version 1.0 or 2.0) and extracts label
% and control point information, which it uses to put together B-spline
% functions using Matlab Spline Toolbox SPMAK.
% This script removes landmarks.

% Example usage:
%       Suppose .cp file is 'wing1000.cp' at 
%           '/Users/surberry/WingMorphology/WingMorphology/Data/Down1F/Gen04'
%       Then call 
%            cp2fun('/Users/surberry/WingMorphology/WingMorphology/Data/Down1F/Gen04',
%                'wing1000.cp')
%       The function takes 2 parameters -- first is the directory the .cp file
%           is in, the second is the actual name of the file
...
% Outputs:
    ...
    % Lab: Alphanumeric labels for wing, read from CP file tag and formattted
        % as a cell array;
...
function [F,LM,Lab,R,F0,scale] = newcp2fun(pathname,filename)

% NOTES for future modification:
    % 1. cannot eliminate scaling - messes with aptknt algorithm
    % 2. placement depends on LM
    % 3. pieces are constructed based on cp input
...
if filename
    ...
    if strcmpi(cph(1:8),'# Image:') %this is a CP ver 1 header
        ...
        % cphdr{1} = ['/Users/surberry/WingMorphology/WingMorphology/Data/Down1F/Gen10','/',fname];
...
%             '# Procru',0;
    ...
    if and(~isempty(nocurves),~isnan(nocurves))
        ...
        for cv = 1:nocurves
            ...
            cp{cv,1} = fscanf(fid,'%f %f',[2 nopoints])'; 
